...
export const get_catalog_image = (dss_name: string, ra: number, dec: number, window_size: number): string => {
    ...
    let url = CATALOG_URL + `/image?position={"ra":${ra},"dec":${dec}}&window-size=${ws}&catalog=${dss_name}&external=1&format=png`
...
export const get_catalog_targets = (catalog_name: string, ra: number, dec: number, radius: number, magRange?: [string, string]): Promise<CatalogTarget[]> => {
    let url = CATALOG_URL + `/sources?position={"ra":${ra},"dec":${dec}}&radius=${radius}&window-size={"size":${radius},"units":"degrees"}&catalog=${catalog_name}&external=1`
    if (magRange) {
